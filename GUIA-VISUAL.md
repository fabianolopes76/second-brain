...
1. **Configuração (01)** → campo **"Vault do Obsidian (destino)"** → 📁 escolha a pasta do seu vault → **Definir**. Fica **lembrado**; Publicar, Auditar vault e Radar passam a apontar para lá. *(Atalho da v3.20: digitar a pasta direto no campo do card Publicar também memoriza — o primeiro uso vira o padrão, e o texto "(padrão: …)" do campo mostra sempre para onde vai.)*
